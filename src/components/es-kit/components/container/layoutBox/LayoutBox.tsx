import { type CSSProperties, type FC, forwardRef, type LegacyRef, memo, useMemo } from 'react'

// import { useParseProps } from '~/components/es-kit/hooks/useParseProps'
import { classNames, parseProps } from '~/components/es-kit/utils/helpers/ui'

import classes from './layoutBox.module.scss'
import { type LayoutBoxProps, LayoutDirection } from './layoutBox.types'
 ...
 */
const resolveFlexProps = (value?: string): string | undefined =>
  value ? flexValueMap[value] || value : value
...
const LayoutBoxRefForwarded = forwardRef(
  (
    ...
    const { dataProps, restProps } = parseProps(props)
    ...
    const resolvedColumn = useMemo(
      () => (column !== undefined && column === true ? LayoutDirection.COLUMN : null),
      [column],
    )
    ...
    const styles = useMemo(() => {
      ...
      if (restProps.direction || resolvedColumn) {
        baseStyles.flexDirection = (restProps.direction ||
          resolvedColumn) as CSSProperties['flexDirection']
      }
    ...
    return (
      <div
        ...
        ref={ref}
        tabIndex={tabIndex}
        className={classNames(classes['layout-box'], className)}
